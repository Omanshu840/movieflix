...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex fixed left-0 top-0 h-screen w-15 bg-gradient-to-b from-slate-900 via-slate-900/90 to-slate-950 border-r border-purple-500/20 flex-col items-center py-8 z-40">
        {/* Main Navigation */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-gradient-to-t from-slate-950 to-slate-900/90 border-t border-purple-500/20 z-40">
        <nav className="flex items-center justify-around py-3 px-2 pb-6">
          {[...mainLinks, ...bottomLinks].map((link) => {
            const Icon = link.icon
            const active = isActive(link.path)
            return (
              <Link
                key={link.path}
                to={link.path}
                className={`p-3 rounded-full transition-all duration-300 flex items-center justify-center w-11 h-11 relative group ${
                  active
                    ? 'bg-gradient-to-br from-purple-500 to-blue-500 text-white shadow-lg shadow-purple-500/50'
                    : 'text-gray-400 hover:bg-slate-800 hover:text-purple-400'
                }`}
          ...
          <button
            onClick={handleSignOut}
            className={`p-3 rounded-full transition-all duration-300 flex items-center justify-center w-10 h-10 relative group text-gray-400 hover:bg-slate-800 hover:text-purple-400`}
            title="Sign Out"
          >
            <FiLogOut className="w-6 h-6" />
            {/* Tooltip */}
